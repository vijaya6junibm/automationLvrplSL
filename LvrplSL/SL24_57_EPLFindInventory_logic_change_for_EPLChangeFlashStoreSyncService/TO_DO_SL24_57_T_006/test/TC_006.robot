...
*** Test Cases ***
TC_006
    [Documentation]  Single Line with FlashPromo='Y' and FlashStores='Y',Seq1 - 40 - NO_INV,Inventory in FLASH (3 stores) - only 2 out of 3 have inventory
    [Tags]      EPLFlashStoreLoadSyncService    REGRESSION
    ...
    Dictionary Should Contain Key     ${findInventoryResponse.json()}     SuggestedOption
         ...
         Log    ${ShipNode}
         Set Test Message    OMS picks best node out of FLASH and 2 FLASH stores will be added to response with OptimizedNode Flag for OMS provided Node
