...
*** Test Cases ***
Multiple order lines where all in backordered status T_1_1.2
    [Documentation]  Multiple order lines where all in backordered status T_1_1.2
    [Tags]      HAPPYFLOW    REGRESSION    CREATEORDER
    Creating Session1     AddData
    ${manageItemJson}=     Manage Item With Dynamic Item ML        ${CURDIR}
    ...
    Dictionary Should Contain Key     ${jsonItemId}     ItemList
         ...
         ${jsonAdjInv}=     Adjust Inventory File ML    ${CURDIR}
    ...
    status should be       204     ${adjInvresponse}
         ${jsonfindInventory}=     Find Inventory File ML       ${CURDIR}
    ...
    ${jsonCreateOrder}=     Create Order File ML      ${CURDIR}
