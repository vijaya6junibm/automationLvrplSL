...
*** Test Cases ***
create order - adjust inv 40 - schedule to 40 - release - backorder from store - adjust inventory in different store and reschedule T_6_6.6
    [Documentation]     create order - adjust inv 40 - schedule to 40 - release - backorder from store - adjust inventory in different store and reschedule
    ...
    ${timestamp} =    Get Current Date    result_format=%Y%m%d-%H%M
         ...
         Set Test Message  Order rescheduled and scheduled against Store where physical inventory available.