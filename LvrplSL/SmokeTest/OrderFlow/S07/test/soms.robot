...
SOMS TC_001
    ...
    Dictionary Should Contain Key     ${response1.json()}     Status
         ...
         ${values}=     Get Value From Json    ${response1.json()}    $..ItemID
            ...
            ${Order_No1}=    Release Input File1     ${Order_No}     ${Order_Header_Key}     ${CURDIR}
         Log To Console        'This is input for release'
         Log To Console        'ord blk1'
         Log To Console        ${Order_No1}
    ${response1}=     Post On Session     AddData   ${base_url_api}${releaseOrder}     data=${Order_No1}    headers=${headers}

     ${Order_No1}=    SOMSNS Input File1     ${Order_No}     "${values}[0]"     ${CURDIR}
         Log To Console        'This is input for release'
         Log To Console        'ord blk1'
         Log To Console        ${Order_No1}
    ...
    Write Output File       ${respSoms}        ${SOMSNS}        ${CURDIR}